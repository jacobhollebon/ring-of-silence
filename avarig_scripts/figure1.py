...
for idx, currSpeakerPositionFile in enumerate(speakerPositionFiles):
    ...
    E_R_sparse = np.zeros(kr.shape) # Energy, reproduced by loudspeaker array with lasso regularisation
    ...
    for i in range((N_dense+1)**2):
        ...
        E_R_sparse += np.abs(Rn * b_lasso[i])**2
    ...
    ax1.plot(kr,10*np.log10(E_R), ls=':', label= 'Reproduced with mode-matching')
    ax1.plot(kr,10*np.log10(E_R_sparse), ls='--', label='Reproduced with Lasso regularisation')
    ylims = [-25, 5] 
    ...
    if save:
        pdf = '.pdf'
        fig1.savefig(saveFolder / f'figure1{pdf}')
